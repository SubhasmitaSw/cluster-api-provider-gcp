...
func (m *MachineScope) InstanceImageSpec() *compute.AttachedDisk {
	...
	if m.GCPMachine.Spec.AcceleratorConfig != nil {
...
func (m *MachineScope) InstanceSpec() *compute.Instance {
	...
	if m.GCPMachine.Spec.AcceleratorConfig != nil {
		instance.GuestAccelerators = []*compute.AcceleratorConfig{
			{
				AcceleratorType:  fmt.Sprintf("projects/%s/zones/%s/acceleratorTypes/%s", m.Project(), m.Zone(), string(m.GCPMachine.Spec.AcceleratorConfig.Type)),
				AcceleratorCount: int64(m.GCPMachine.Spec.AcceleratorConfig.Count),
			},
		}
	}
